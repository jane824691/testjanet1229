...
import { AB_LIST3, AB_COUNT_COMMENT } from "@/components/my-const_forum";
...
let page= +router.query.page || 1;
  if(page<1) page=1;
  try {
    const r =await fetch(AB_LIST3+`?page=${page}&keyword=${encodeURIComponent(myForm.article_content)}`);
...
const yourSubmitFunction = async (e) => {
  ...
  try {
    ...
    console.log('page',router.query.page,'keyword',encodedKeyword);
      const r = await fetch(AB_LIST3 + `?page=${router.query.page}&keyword=${encodedKeyword}`);
      const d = await r.json();
      setData(d);
  } catch (ex) {
      console.error(ex);
