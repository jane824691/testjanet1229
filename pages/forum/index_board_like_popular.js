...
import { AB_LIST8, AB_COUNT_COMMENT } from "@/components/my-const_forum";
...
let page= +router.query.page || 1;
  if(page<1) page=1;
  try {
    const r =await fetch(AB_LIST8+`?page=${page}&keyword=${encodeURIComponent(myForm.article_content)}`);
...
const yourSubmitFunction = async (e) => {
  ...
  try {
    ...
    console.log('page',router.query.page,'keyword',encodedKeyword);
      const r = await fetch(AB_LIST8 + `?page=${router.query.page}&keyword=${encodedKeyword}`);
      const d = await r.json();
      setData(d);
  } catch (ex) {
      console.error(ex);
...
{/* 左邊欄位 */}
  ...
  {/* 搜尋 */}{/* 搜尋 */}{/* 搜尋 */}{/* 搜尋 */}{/* 搜尋 */}{/* 搜尋 */}
          ...
          <h4 className={styles.titlePopular}>
              <Link href={`/forum/index_board_like`} className={styles.articleCardLink}>
            ...
            <h4 className={styles.titlePopular}>
              <Link href={`/forum/index_board_like_popular`} className={styles.articleCardLink}>
